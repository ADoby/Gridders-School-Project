...
public class ThirdPersonCharacter : MonoBehaviour {
	...
	void PreventStandingInLowHeadroom ()
	{
		...
		if (!crouchInput)
		{
           // Debug.Log("PreventStandingInLowHeadroom rigidbody=" + rigidbody);
           // Debug.Log("PreventStandingInLowHeadroom capsule=" + capsule);
            Ray crouchRay = new Ray(transform.position, Vector3.up);
            RaycastHit hit;
            if (Physics.Raycast(crouchRay, out hit, originalHeight))
            {
